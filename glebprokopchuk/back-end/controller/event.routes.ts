...
eventRouter.get('/', async (req: Request, res: Response, next: NextFunction) => {
    try {
        // Get all events - Gleb
        const events: any = await eventService.getAllEvents();
...
eventRouter.get(
    ...
    try {
        // Extract organiserId from request parameters - Gleb
        const organiserId = parseInt(req.params.organiserId);
        const event = await eventService.getEventsByOrganiserId({organiserId});
...
eventRouter.post('/', async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        //create event here - Gleb
        const event = await eventService.createEvent({name, description, date: new Date(date), location}, user);
